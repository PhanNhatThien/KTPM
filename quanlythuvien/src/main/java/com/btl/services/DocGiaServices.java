...
public class DocGiaServices {
    public List<DocGia> getDSDocGia(String kw) throws SQLException {
        try (Connection conn = JdbcUtils.getConn()) {
            PreparedStatement stm = conn.prepareStatement("SELECT * FROM `doc_gia` "
                    + "WHERE (ho_ten like concat('%',?,'%')) OR (ma_doc_gia like concat('%',?,'%')) OR (gioi_tinh like concat('%',?,'%'))");
            if (kw == null)
                kw = "";
    ...
    public boolean dangKy(DocGia docGia) throws SQLException {
        if(docGia.getMaDocGia() != null && docGia.getMaDocGia().isBlank() == false && checkDocGiaExist(docGia.getMaDocGia()) == false){
            try (Connection conn = JdbcUtils.getConn()){
